<template>
    <el-dialog title="图库" width="80%" :visible.sync="imgVisible">
        <div style="margin:0 auto;">
            <el-upload class="upload-demo" :on-success="onSuccess" :show-file-list="false" :action="url+'visualsDetail/upload'" multiple list-type="picture">
                <el-button size="small" icon="el-icon-upload" type="primary">点击上传</el-button>
            </el-upload>
        </div>
        <el-scrollbar class="imgList">
            <img :src="item.value" :style="styleName" @click="handleSetimg(item.value)" v-for="(item,index) in imgOption[imgActive]" :key="index" />
        </el-scrollbar>
    </el-dialog>
</template>
